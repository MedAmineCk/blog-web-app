...
include_once $url . '/php-rest-api/class/Clients.php';
...
// Upload directory
$upload_location = $url . "/uploads/clients/";
    ...
    if (in_array($ext, $valid_ext)) {
        if ($_FILES['file']['error'] === 0) {
            if ($_FILES['file']['size'] < 1000000) {
                ...
                $sqlQuery = "SELECT profile_pic from clients WHERE id_client =  '$profileId'";
                ...
                if (move_uploaded_file($_FILES['file']['tmp_name'], $path)) {
                    $client->id_client = $profileId;
                    $client->profile_pic = $fileNameNew;
                    if ($client->updateClientProfile()) {
                        echo true;
                    } else {
