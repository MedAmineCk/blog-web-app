...
include_once $url . '/php-rest-api/class/Confirmers.php';
...
// Upload directory
$upload_location = $url . "/uploads/confirmers/";
    ...
    if (in_array($ext, $valid_ext)) {
        if ($_FILES['file']['error'] === 0) {
            if ($_FILES['file']['size'] < 1000000) {
                ...
                $sqlQuery = "SELECT profile_pic from confirmers WHERE id_confirmer =  $profileId";
                ...
                if (move_uploaded_file($_FILES['file']['tmp_name'], $path)) {
                    $confirmer->id_confirmer = $profileId;
                    $confirmer->profile_pic = $fileNameNew;
                    if ($confirmer->updateConfirmerProfile()) {
                        echo true;
                    } else {
